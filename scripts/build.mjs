...
const required = ["id", "name", "description", "url", "sitemapUrl", "status", "icon", "featured", "order", "stats"];
const ids = new Set();
for (const project of projects) {
  ...
  if (!/^https:\/\//.test(project.sitemapUrl)) throw new Error(`${project.id}: sitemap URL must use HTTPS`);
  ids.add(project.id);
}
...
const sitemapUrls = [portalSitemapUrl, ...projects.filter(project => project.status === "Live").map(project => project.sitemapUrl)];
...
const gamesIcon = `<svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M7.5 8h9a4 4 0 0 1 3.8 2.8l1.1 3.5a3 3 0 0 1-5 3l-1.2-1.3H8.8l-1.2 1.3a3 3 0 0 1-5-3l1.1-3.5A4 4 0 0 1 7.5 8Z"/><path d="M8 11v4M6 13h4M16.5 12.5h.01M18.5 14.5h.01"/></svg>`;
const icons = { tools: toolsIcon, games: gamesIcon };
...
const description = "Explore AllTools browser utilities and AWI Games, independent digital projects built and published by Tutku Tuzlu.";
const home = shell({title:"Tutku Tuzlu Projects – Tools and Browser Games",description,body:`<section class="hero"><div class="container hero-inner"><p class="eyebrow">Tutku Tuzlu Projects</p><h1>Independent digital projects, tools and publications.</h1><p class="lead">Useful products built and published by Tutku Tuzlu.</p></div></section><section class="section" id="projects"><div class="container"><div class="section-heading"><p class="eyebrow">Projects</p><h2>Available now</h2><p>Focused digital products that are live and ready to use.</p></div><div class="project-grid">${cards}</div></div></section>`,structured:{"@context":"https://schema.org","@type":"ProfilePage",name:"Tutku Tuzlu Projects",description,url:"https://tutkutuzlu.github.io/",mainEntity:{"@type":"Person",name:"Tutku Tuzlu",url:"https://tutkutuzlu.github.io/"},hasPart:projectJsonLd}});
